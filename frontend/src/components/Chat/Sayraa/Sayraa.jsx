...
const Chat = ({ isFloating = false, onClose = null }) => {
  ...
  // Define speakText: always uses the exact cute, sweet female greeting voice
  ...
  const speakText = useCallback((text) => {
    ...
    const processedText = cleanedText.replace(/Sayraa/gi, "सायरा");
    let lang = "hi-IN";
  ...
  // Load messages and setup Speech Recognition on mount
  useEffect(() => {
  ...
  const startListening = async () => {
    ...
    try {
      ...
      alert("Please allow microphone access to speak with Sayraa.");
    }
  };
  ...
  const medConfig = {
    ...
    systemMessage: `You are Sayraa, an empathetic, polite, caring, and loving female healthcare AI assistant:
  ...
  const sendMessage = async (input = userInput) => {
    ...
    try {
      ...
      // Primary language is Hinglish (unless Odia script is explicitly detected)
      let detectedLang = /[\u0B00-\u0B7F]/.test(input) ? "Odia" : "Hinglish";

      console.log("Sayraa speaking in language:", detectedLang);

      const languageInstruction = detectedLang === "Odia"
        ? `Respond in Odia script. STRICT LENGTH RULE: Keep answer between 2 lines to MAXIMUM 4 lines only. Always be deeply polite, gentle, and caring. NEVER say you are redirecting the user to doctor appointments or other pages.`
        : `MANDATORY LANGUAGE & EMPATHY: You MUST respond in sweet, conversational, loving HINGLISH (Hindi written in Roman English script). NEVER tease, mock, or call the user a fool under any circumstance. When user reports fever ("bukhar hua") or any sickness/sprain/injury (e.g. "pair mein moch"), immediately show genuine sympathy (e.g. "Aww, apna khayal rakho! Thoda rest karo aur paani piyo..."). Keep medical terms in English. 
CRITICAL MANDATE: DO NOT write "main aapko doctor appointment section mein redirect kar rahi hoon" or state that you are redirecting the user to doctor appointment or any other section. Do not promise or mention redirecting in your chat reply.
STRICT LENGTH RULE: Exactly 2 to 4 lines only.
